...
const wss = new WebSocket.Server({ port: process.env.SOCKET_PORT || "80" })
...
app.get('/tasks', async (req, res) => {
  ...
  const token = ""  //TODO update lock token
  const result = ""  //TODO send api call
  ...
  let deletedTasks = []
  if (req.query.since) {
    const rows = []  //TODO send an API call to compare
    deletedTasks = rows[0].map(item => item._id)
  }
...
app.put('/issues/id/:id', async (req, res) => {
  try {
    if (!req.body.title || !req.body.text) {
      throw new StatusError('Title and Text fields must be filled', 400)
    }
    const task = await checkIfTaskIsEditable(req.params._id, req)
    const dt = (new Date).getTime()
